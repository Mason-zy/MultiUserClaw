...
def _create_app(adapter: APIServerAdapter) -> web.Application:
    ...
    app.router.add_get("/api/hermes/sessions", adapter._handle_list_sessions)
    app.router.add_get("/api/hermes/sessions/{session_id}", adapter._handle_get_session)
    app.router.add_put("/api/hermes/sessions/{session_id}/title", adapter._handle_rename_session)
    app.router.add_delete("/api/hermes/sessions/{session_id}", adapter._handle_delete_session)
    app.router.add_get("/api/hermes/events/stream", adapter._handle_events_stream)
    return app
...
class TestHermesSessionCompatEndpoints:
    @pytest.mark.asyncio
    async def test_list_sessions_returns_compat_payload(self, adapter):
        class FakeDB:
            def list_sessions_rich(self, limit=100):
                assert limit == 100
                return [{
                    "id": "sess-1",
                    "title": "First",
    ...
    async def test_get_session_returns_messages(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.get("/api/hermes/sessions/sess-2")
            assert resp.status == 200
            data = await resp.json()
    ...
    @pytest.mark.asyncio
    async def test_rename_session_updates_title(self, adapter):
        class FakeDB:
            def resolve_session_id(self, session_id):
                return session_id
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.put("/api/hermes/sessions/sess-3/title", json={"title": "Renamed"})
            assert resp.status == 200
            data = await resp.json()
            assert data == {"ok": True, "session_id": "sess-3", "title": "Renamed"}

    @pytest.mark.asyncio
    async def test_delete_session_returns_ok(self, adapter):
        class FakeDB:
            def resolve_session_id(self, session_id):
                return session_id

            def delete_session(self, session_id):
                assert session_id == "sess-4"
                return True
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.delete("/api/hermes/sessions/sess-4")
            assert resp.status == 200
            data = await resp.json()
            assert data == {"ok": True, "session_id": "sess-4"}
