...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Subscribers for the global compatibility event stream.
        self._event_subscribers: set["asyncio.Queue[Optional[Dict[str, Any]]]"] = set()
    ...
    def _resolve_session_id(self, session_id_or_prefix: str) -> Optional[str]:
    ...
    def _session_summary_payload(self, session: Dict[str, Any]) -> Dict[str, Any]:
        updated_at = self._format_timestamp(session.get("last_active") or session.get("started_at"))
        created_at = self._format_timestamp(session.get("started_at"))
        return {
    ...
    def _session_detail_payload(self, session: Dict[str, Any], messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        payload = self._session_summary_payload(session)
        payload["messages"] = messages
        payload["source"] = session.get("source")
        payload["model"] = session.get("model")
        payload["ended_at"] = self._format_timestamp(session.get("ended_at"))
        return payload

    def _broadcast_event(self, event: Dict[str, Any]) -> None:
        stale: List["asyncio.Queue[Optional[Dict[str, Any]]]"] = []
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"sessions": [self._session_summary_payload(session) for session in sessions]})
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        session_id = self._resolve_session_id(requested)
        try:
            session = db.get_session(session_id) if session_id else None
            if not session:
                return web.json_response(_openai_error(f"Session not found: {requested}", code="session_not_found"), status=404)
        ...
        return web.json_response(self._session_detail_payload(session, messages))
    ...
    async def _handle_rename_session(self, request: "web.Request") -> "web.Response":
        ...
        if not updated:
            return web.json_response(_openai_error(f"Session not found: {requested}", code="session_not_found"), status=404)
        return web.json_response({"ok": True, "session_id": session_id, "title": db.get_session_title(session_id)})
    ...
    async def _handle_events_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        try:
            ...
            logger.debug("[api_server] global event stream closed: %s", exc)
        finally:
            self._event_subscribers.discard(queue)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_get("/api/hermes/sessions", self._handle_list_sessions)
            self._app.router.add_get("/api/hermes/sessions/{session_id}", self._handle_get_session)
            self._app.router.add_put("/api/hermes/sessions/{session_id}/title", self._handle_rename_session)
            self._app.router.add_delete("/api/hermes/sessions/{session_id}", self._handle_delete_session)
            self._app.router.add_get("/api/hermes/events/stream", self._handle_events_stream)
