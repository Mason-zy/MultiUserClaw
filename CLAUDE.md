...
- 「写到记忆文件」= 本项目 `.claude/memory.md`——**精简索引**（核心结论 + `file:line` + 状态，编号连续），**不写长文**。新条目在「关联记忆」前插入。
- 详细过程/方案评估/验证步骤/架构图 → `.claude/memory/archive-*.md`（5 主题分片，入口 `README.md`）。不写到 `~/.claude/` 全局记忆，不散落到 `doc/`。
- 待做/核实清单固定放项目根：`代做（核实）清单.md`
- 分析报告固定放 `doc/Analysis/YYYY/MM/DD/`，例如 `doc/Analysis/2026/07/03/`
- 新会话启动后，如需回溯历史，从 `.claude/memory.md` 找摘要；细节按主题读对应的 `archive-*.md`。
